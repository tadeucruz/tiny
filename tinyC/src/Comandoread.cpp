...
#include "../include/Comandoread.h"

Comandoread::Comandoread(int linha,Memoria* memoria,string smemoria){
    m = memoria;
    this->linha = ++linha;
    codmemoria = smemoria[0];
}

int Comandoread::executa(void){
    double valor;
    cin >> valor;
    m->setMemoria(codmemoria,valor);
    return linha;
}