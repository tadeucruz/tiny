...
#include "../include/ComandowriteVar.h"

ComandowriteVar::ComandowriteVar(int linha,Memoria* memoria,string smemoria){
    m = memoria;
    this->linha = ++linha;
    codmemoria = smemoria[0];
}

int ComandowriteVar::executa(void){
    double v = m->getMemoria(codmemoria);
    cout << v;
    return linha;
}